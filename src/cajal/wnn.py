...
class Modality:
    ...
    def __init__(self, obsv: npt.NDArray[np.float_]):
        ...
        self.nn_distance = self.dmat[:, self.nn_index_arr[:, 1]]

    @staticmethod
    def of_dmat(dmat: DistanceMatrix, intrinsic_dim: Optional[int] = None, n_neighbors=20):
        ...
        m.nn_distance = m.dmat[:, m.nn_index_arr[:, 1]]
        return m
...
def wnn(modalities: list[Modality], k: int, epsilon: float = 1e-4):
    ...
    def cross_modality_affinities(
        ...
        return np.exp(
            ...
            / (m.local_bandwidth - m.nn_distance)
        )

    def all_pairwise_affinities(modalities: list[modalities]):
        ...
        for i in range(M):
            for j in range(M):
                ...
                pairwise_affinity_ratios[i, k, :] = pairwise_affinities[i, i, :] / (
                    pairwise_affinities[i, k, :] + epsilon
                )
        return softmax(pairwise_affinity_ratios, axis=1)

    return all_pairwise_affinities(modalities)
