open Bigarray
module Unbalanced_gw = Unbalanced_gw_multicore
...
module Squareform : sig
  ...
  (* val arr_to_bigarray : arr -> (float, float64_elt, c_layout) Genarray.t *)
  (* val t_to_bigarray: t -> (float, float64_elt, c_layout) Genarray.t *)
  ...
  val unbalanced_gw_pairwise_f64: Context.t -> arr -> params ->
    (float, float64_elt, c_layout) Genarray.t
  (* val unbalanced_gw_f64_increasing_eps: Context.t -> *)
  (*   t -> Array_f64_1d.t -> t -> Array_f64_1d.t -> params -> *)
  (*   (float, float64_elt, c_layout) Genarray.t *)
  val t_of_bigarray: (float, float64_elt, c_layout) Genarray.t -> t
  (* val bigarray_of_t: t -> (float, float64_elt, c_layout) Genarray.t *)
  
  val arr_of_bigarray: (float, float64_elt, c_layout) Genarray.t -> arr

  val slice_left: arr -> int -> t

  val unbalanced_gw_armijo_pairwise : 
  Context.t -> 
  arr ->
  params -> 
  (float, float64_elt, c_layout) Genarray.t
  ...
  (* let t_to_bigarray arr = arr *)

  let unbalanced_gw_f64 ctx (x: t) mu (y: t) nu params =
    let x = Unbalanced_gw.Array_f64_2d.v ctx x in
    let y = Unbalanced_gw.Array_f64_2d.v ctx y in
    Unbalanced_gw.unbalanced_gw_total_cost
        ctx params.rho1 params.rho2 params.epsilon x mu y nu
  ...
  let unbalanced_gw_naive_descent ctx ~rho1 ~rho2 ~eps a_dmat mu b_dmat nu ~tau ~c ~tol_outerloop =
    let open Unbalanced_gw in
    let a_dmat = Array_f64_2d.v ctx a_dmat in
    let b_dmat = Array_f64_2d.v ctx b_dmat in
    Unbalanced_gw.ugw_naive ctx rho1 rho2 eps a_dmat mu b_dmat nu tau c tol_outerloop
  ...
  let unbalanced_gw_pairwise_f64 ctx (dmat_array: arr) params =
  let open Unbalanced_gw in
  let num_pt_clouds = num_spaces dmat_array in
  let m = (num_pt_clouds * (num_pt_clouds-1))/2 in 
  let main_input = Array_f64_3d.v ctx dmat_array in
  let results =
    unbalanced_gw_pairwise ctx main_input
      params.rho1 params.rho2 params.epsilon params.exp_absorb_cutoff
      params.safe_for_exp params.tol_sinkhorn params.tol_outerloop
  in
  ...
  let unbalanced_gw_armijo_pairwise ctx arr params =
    let open Unbalanced_gw in
    let dmats = Array_f64_3d.v ctx arr in
    Unbalanced_gw.ugw_armijo_pairwise
...
module Pt_cloud : sig
  ...
  let unbalanced_gw_pairwise_f64 ctx (pt_cloud_array: arr) params =
    let open Unbalanced_gw in
    let num_pt_clouds = num_spaces pt_cloud_array in
    let m = (num_pt_clouds * (num_pt_clouds-1))/2 in 
    let main_input = Array_f64_3d.v ctx @@ pt_cloud_array in
    let results =
      unbalanced_gw_pairwise_pt_clouds ctx main_input
        params.rho1 params.rho2 params.epsilon params.exp_absorb_cutoff
        params.safe_for_exp params.tol_sinkhorn params.tol_outerloop
    in
    let output = Genarray.create Float64 c_layout [| m; 5 |] in
  ...
  let unbalanced_gw_f64 ctx x u y v params =
    Squareform.unbalanced_gw_f64 ctx (pdist x) u (pdist y) v params;;
end
