 ...
 =
    ...
    if sumC' M.> zero then
      ...
      let (diff, loss, _) =
      	...
      	-- let gradient = (gradient_fn a.C) in
        -- assert (not (any (any M.isnan) (#[trace] gradient))) (
      	let diff = (map2 (map2 (M.-)) x a.C) in
      	-- assert (not (any (any M.isnan) (#[trace] diff))) (
      	-- let g = frobenius gradient diff in
        -- assert (not (M.isnan (#[trace] g))) (
