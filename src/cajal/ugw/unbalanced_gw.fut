...
entry ugw_armijo_pairwise_increasing [k][m]
  (ugw_dmat:[][5]f64)(ratio:f64)
  rho1 rho2 eps (A: [k][m][m]f64.t) (distrs: [k][m]f64.t)
  exp_absorb_cutoff safe_for_exp tol_sinkhorn tol_outerloop =
  pairsf64.pairs k
  |> map (\(i,j) ->
	    ...
	    let arr =
	      unbalanced_gw64.armijo.main
	      rho1 rho2 increase_eps
	      A[i] distrs[i] A[j] distrs[j]
	      {exp_absorb_cutoff, loop_count=1, safe_for_exp, tol_sinkhorn}
	      tol_outerloop
	    in
	    (arr, increase_eps)
	 ) |> unzip |> (\o -> o.0)
